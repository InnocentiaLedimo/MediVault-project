...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO appointments (appointmentDate, appointmentTime, appointmentDetails, appointmentFirstName, appointmentLastName) 
            VALUES ('$date', '$time', '$details', '$firstName','$lastName')";

    if ($connection->query($sql) === TRUE) {
        echo "<script>alert('Appointment booked successfully!'); window.history.back();</script>";
        exit();
    } else {
        echo "<script>alert('Database error: " . $connection->error . "'); window.history.back();</script>";
        exit();
    }
